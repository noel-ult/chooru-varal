...
export function HandCalibration({ onComplete, onDemo }: { onComplete: (value: VaariEstimate) => void; onDemo: () => void }) {
  ...
  return <section>
    <div className="mb-7"><p className="eyebrow">01 / HAND GEOMETRY ANALYSIS</p><h2 className="mt-2 text-4xl font-black tracking-tight">Show your hand.</h2><p className="mt-2 text-[#050505]">Let&apos;s scientifically measure your vaari.</p></div>
    <div className="panel overflow-hidden rounded-3xl"><div className="relative aspect-video bg-[#050505]"><video ref={videoRef} muted playsInline className="h-full w-full scale-x-[-1] object-cover"/><div className="pointer-events-none absolute inset-y-[14%] left-[37%] right-[37%] rounded-[45%] border-2 border-dashed border-[#F5009F] opacity-80"><span className="absolute -bottom-7 left-1/2 -translate-x-1/2 whitespace-nowrap text-[10px] font-bold tracking-widest text-[#ffdccb]">PLACE ONE OPEN HAND HERE</span></div><canvas ref={canvasRef} className="pointer-events-none absolute inset-0 h-full w-full scale-x-[-1]"/>{error && <div className="absolute inset-0 grid place-items-center bg-[#050505] p-8 text-center text-white"><div><b>Camera permission needed.</b><p className="mt-2 text-sm text-white/70">{error}</p><button onClick={() => location.reload()} className="mt-5 rounded-full bg-white px-4 py-2 text-sm font-bold text-black">Retry camera</button></div></div>}<div className="absolute bottom-4 left-4 rounded-full bg-black/65 px-4 py-2 text-sm font-bold text-white"><span className="mr-2 inline-block h-2 w-2 rounded-full bg-[#61d981]"/>{status}</div></div><div className="flex flex-wrap items-center justify-between gap-4 p-5"><div><p className="eyebrow">ESTIMATED VAARI CAPACITY</p><b className="text-2xl">{estimate ? `${estimate.grams} g` : "Awaiting landmark data"}</b></div><div className="flex gap-2"><button onClick={() => { onComplete(demoVaari); onDemo(); }} className="rounded-full border border-[#128EEF] px-4 py-2 text-sm font-bold">Use demo hand</button><button disabled={!estimate} onClick={() => estimate && onComplete(estimate)} className="rounded-full bg-[#050505] px-5 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-30">Continue →</button></div></div></div>
  </section>;
}
