...
export function VaariMotionCapture({ hand, onComplete, onDemo }: { hand: VaariEstimate; onComplete: (x: MotionProfile) => void; onDemo: () => void }) {
  ...
  return <section><p className="eyebrow">03 / VAARI MOTION</p><h2 className="mt-2 text-4xl font-black tracking-tight">Show us how you take one vaari.</h2><p className="mt-2 text-[#050505]">A heuristic motion study of your natural scoop. Start, scoop, lift, hold, release.</p><div className="mt-7 grid gap-5 md:grid-cols-[1fr_.9fr]"><div className="panel overflow-hidden rounded-3xl"><div className="relative aspect-video bg-[#050505]"><video ref={video} muted playsInline className="h-full w-full scale-x-[-1] object-cover"/><div className="absolute bottom-4 left-4 rounded-full bg-black/65 px-4 py-2 text-sm font-bold text-white">{recording?"● CAPTURING MOTION":message}</div></div><div className="flex gap-3 p-5"><button onClick={toggle} className={`rounded-full px-5 py-3 text-sm font-bold text-white ${recording?"bg-[#F5009F]":"bg-[#050505]"}`}>{recording?"Stop capture":"Start motion capture"}</button><button onClick={onDemo} className="rounded-full border border-[#128EEF] px-4 py-2 text-sm font-bold">Use demo motion</button></div></div><div className="panel rounded-3xl p-5">{points.length>1?<TrajectoryView points={points}/>:<div className="grid h-full min-h-64 place-items-center text-center text-sm text-[#74766f]">Your recorded wrist path will appear here.<br/>No fake trajectory, only tracked landmark data.</div>}<button disabled={points.length<12} onClick={()=>points.length>=12&&onComplete(analyzeMotion(points,hand.grams))} className="mt-5 w-full rounded-full bg-[#050505] py-3 text-sm font-bold text-white disabled:opacity-40">Analyze motion →</button></div></div></section>;
}
